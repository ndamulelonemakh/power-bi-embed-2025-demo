...
import { sampleTheme } from "./constants";
...
function App() {
  ...
  const [isEmbedConfigDialogOpen, setIsEmbedConfigDialogOpen] =
    useState<boolean>(false);
  const [isFilterPaneVisibleAndExpanded, setIsFilterPaneVisibleAndExpanded] =
    useState<boolean>(true);
  const [isThemeApplied, setIsThemeApplied] = useState<boolean>(false);
  const [isZoomedOut, setIsZoomedOut] = useState<boolean>(false);
  const [isDataSelectedEvent, setIsDataSelectedEvent] =
    useState<boolean>(false);
  const [isEventDetailsDialogVisible, setIsEventDetailsDialogVisible] =
    useState<boolean>(false);
  const [dataSelectedEventDetails, setDataSelectedEventDetails] =
    useState<any>(null);
   ...
   */
  const toggleFilterPane = async (): Promise<
    IHttpPostMessageResponse<void> | undefined
  > => {
    if (!report) {
    ...
    try {
      const response: IHttpPostMessageResponse<void> =
   ...
   */
  const dataSelectedEventDetailsDialog = (
  ...
  const toggleTheme = async (): Promise<void> => {
    ...
    try {
      (await isThemeApplied)
        ? report.resetTheme()
        : report.applyTheme({ themeJson: sampleTheme });
  ...
  const refreshReport = async (): Promise<void> => {
    ...
    try {
      ...
      setDisplayMessageAndConsole(
        "The report has been refreshed successfully."
      );
    } catch (error: any) {
  ...
  const controlButtons = isEmbedded ? (
    <>
      <div className="button-container">
        ...
        <button onClick={toggleTheme}>
          {isThemeApplied ? "Reset theme" : "Set theme"}
        </button>
        ...
        <button onClick={enableFullScreen}>Full screen</button>
      </div>
    ...
    <>
      ...
      <button
        className="embed-report"
        onClick={() => setIsEmbedConfigDialogOpen(true)}
      >
        Embed Report
      </button>
    </>
  ...
  const footer = (
    <div
      className="footer"
      aria-hidden={isEmbedConfigDialogOpen || isEventDetailsDialogVisible}
    >
