  ...
  return isOpen ? (
    <FluentDialog>
      <div className="dialog-header">
        ...
        <button className="close-icon-button" onClick={hideEmbedConfigDialog}>
          &#x2715;
        </button>
      </div>
      <div className="dialog-main">
        <p>
          Follow the{" "}
          <a
            href="https://learn.microsoft.com/rest/api/power-bi/embed-token/generate-token"
            target="_blank"
          ...
          documentation to generate a Microsoft Entra Token.
        </p>
        <span>Insert your Microsoft Entra token</span>
        <FluentTextField
          name="aadToken"
          value={aadToken}
          onInput={onAadTokenChange}
          className="dialog-field"
          aria-label="AAD Token"
        />

        <p>
          Use the{" "}
          <a
            href="https://learn.microsoft.com/rest/api/power-bi/reports/get-report-in-group"
            target="_blank"
            rel="noopener noreferrer"
          >
            Get Report In Group
          </a>{" "}
          REST API to get your embed URL.
        </p>
        <span>Insert your embed URL</span>
        <FluentTextField
          name="embedUrl"
          value={embedUrl}
          onInput={onEmbedUrlChange}
          className="dialog-field"
      ...
      <div className="dialog-buttons">
        <FluentButton
          className={`run-button ${areFieldsFilled ? "active" : ""}`}
          disabled={!areFieldsFilled}
          onClick={runConfig}
        >
          Run
        </FluentButton>
        <FluentButton className="close-button" onClick={hideEmbedConfigDialog}>
          Close
        </FluentButton>
      </div>
    </FluentDialog>
